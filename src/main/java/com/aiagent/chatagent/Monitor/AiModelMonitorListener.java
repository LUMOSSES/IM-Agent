package com.aiagent.chatagent.Monitor;
...
public class AiModelMonitorListener implements ChatModelListener {
    ...
    public void onRequest(ChatModelRequestContext requestContext) {
        ...
        if (context == null) {
            // MCP工具调用等内部子请求运行在线程池上，ThreadLocal无法传递，静默跳过
            log.debug("MonitorContext is null, skipping monitoring for this sub-call");
            return;
        }
    ...
    public void onResponse(ChatModelResponseContext responseContext) {
        ...
        if (context == null) {
            // MCP工具调用等内部子请求，对应onRequest也未设置上下文，静默跳过
            log.debug("监控上下文丢失，跳过响应监控 - Model: {}", responseContext.chatResponse().modelName());
            return;
        }
